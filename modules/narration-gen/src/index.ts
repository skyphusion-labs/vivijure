...
//
// ASYNC: synthesis can take tens of seconds, so (mirrors cast-image -- /poll does the work, NOT a
// background ctx.waitUntil which the runtime cancels ~30s after the response, hanging the job, #155):
...
async function submit(
  ...
  try {
    // No synthesis here: persist a `pending` state carrying the inputs and let /poll do the work (#155).
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<ScoreOutput>> {
  ...
  if (state.status === "pending") {
    // First poll claims the job and runs synthesis inline, within this request's own budget. The claim
    ...
    const done = await runSynthesis(env, token.job_id, state.input, state.config);
    if (done.status === "failed") return { ok: false, error: done.error || "synthesis failed" };
