// openai-sora: a motion.backend module worker (vivijure-module/2), OpenAI Sora 2 I2V on RunPod.
...
import { buildSoraBody, extractVideoUrl, clipKey, clampDuration, encodePoll, decodePoll, runpodJobGone, classifyGoneState, workersStillCold, terminalErrorInOutput, RUNPOD_COLD_GRACE_MS } from "./sora";
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (s.status !== "COMPLETED") {
    ...
    if (backendErr) {
      ...
      return { ok: false, error: "openai-sora backend error (job " + st.jobId + ", status stuck " + String(s.status ?? "unknown") + ", cancel issued): " + backendErr };
    }
