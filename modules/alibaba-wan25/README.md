...
A **`motion.backend`** module (vivijure-module/2): the **Alibaba Wan 2.5**
...
- `RUNPOD_API_KEY` -- the RunPod API key for the endpoint. Use a DEDICATED, scoped vivijure key (one
  ...
  `npx wrangler secret put RUNPOD_API_KEY -c modules/alibaba-wan25/wrangler.toml`.
...
**Model / endpoint**: fixed in code -- `ENDPOINT = https://api.runpod.ai/v2/wan-2-5`. Selecting a
