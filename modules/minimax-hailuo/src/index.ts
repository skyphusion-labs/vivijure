...
import { buildHailuoBody, extractVideoUrl, clipKey, clampDuration, encodePoll, decodePoll, runpodJobGone, classifyGoneState } from "./hailuo";
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!env.RUNPOD_API_KEY) return { ok: false, error: "minimax-hailuo: RUNPOD_API_KEY not configured" };
  try {
