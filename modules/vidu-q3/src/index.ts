// vidu-q3: a motion.backend module worker (vivijure-module/1), Vidu Q3 I2V on RunPod. ASYNC:
...
import { buildViduBody, extractVideoUrl, clipKey, clampDuration, encodePoll, decodePoll, runpodJobGone, classifyGoneState } from "./vidu";
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!env.RUNPOD_API_KEY) return { ok: false, error: "vidu-q3: RUNPOD_API_KEY not configured" };
  try {
