...
#    Reads deploy.env (VIVIJURE_PROFILE = minimal | full). This is the Cloudflare Worker + D1 + R2
...
#    Extra RunPod endpoints behind the studio full profile (finish-lipsync / finish-upscale /
#    speech-upscale). Stood up from their own repos, then set the endpoint ids in deploy.env and
#    re-run the studio with VIVIJURE_PROFILE=full.
say "[4/4] finish satellites -- PLACEHOLDER (per-repo endpoints; then studio VIVIJURE_PROFILE=full)"
